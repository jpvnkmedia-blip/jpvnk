...
<head>
    <meta charset="UTF-8">
    <title>BORANG D - PEMBATALAN PENDAFTARAN TERNAKAN RUMINAN AKIBAT SEMBELIHAN ({{ $permit->no_permit }})</title>
...
<body class="p-4 sm:p-8 flex flex-col items-center">
    ...
    <div class="no-print mb-6 flex flex-wrap gap-3">
        <button onclick="window.print()" class="px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-sm rounded-xl shadow-lg transition flex items-center gap-2">
            ...
            <span>Cetak Borang D (Format Asal)</span>
        </button>
        <a href="{{ route('eptr.borang-d.show', $permit->id) }}" class="px-4 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold text-sm rounded-xl transition">
            Kembali ke Maklumat Permit
        </a>
    </div>
    ...
    <!-- Official Borang D Sheet (Exact Gazette Document Format) -->
    <div class="form-container max-w-[210mm] w-full bg-white p-10 sm:p-14 shadow-2xl text-black">
