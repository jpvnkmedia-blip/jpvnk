...
<head>
    <meta charset="UTF-8">
    <title>CETAK PUKAL SKV SEMBELIH - JPVNK</title>
...
<body class="p-4 sm:p-6 flex flex-col items-center">
    ...
    <div class="no-print mb-6 flex flex-wrap gap-3">
        <button onclick="window.print()" class="px-6 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-sm rounded-xl shadow-lg transition flex items-center gap-2">
            ...
            <span>Cetak Pukal SKV Sembelih ({{ count($permitList) }} Set)</span>
        </button>
        <a href="{{ route('eptr.borang-d.index') }}" class="px-4 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold text-sm rounded-xl transition">
            Kembali ke Senarai Permit
        </a>
    </div>
    ...
    <div class="max-w-[210mm] w-full space-y-6">
        @foreach($permitList as $index => $permit)
            ...
            <!-- ================= HALAMAN 1 (FORMAT WARTA) ================= -->
            <div class="form-page p-8 sm:p-12 shadow-2xl bg-white border border-slate-200 {{ $index > 0 ? 'page-break' : '' }}">
