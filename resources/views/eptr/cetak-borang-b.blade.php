...
<head>
    <meta charset="UTF-8">
    <title>BORANG B - PERAKUAN DAFTAR TERNAKAN RUMINAN ({{ $pindahMilik->ternakan->no_tag ?? 'ID-'.$pindahMilik->id }})</title>
...
<body class="p-4 sm:p-8 flex flex-col items-center">
    ...
    <div class="no-print mb-6 flex flex-wrap gap-3">
        ...
        <a href="{{ route('eptr.borang-b.show', $pindahMilik->id) }}" class="px-4 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold text-sm rounded-xl transition">
            Kembali ke Butiran Borang B
        </a>
    </div>
    ...
    <div class="form-container max-w-[210mm] w-full p-8 sm:p-12 shadow-2xl text-black bg-white">
        ...
        @php
            ...
            $pemunyaAsal = $ternakan ? $ternakan->pemunya_asal : $pindahMilik->pemunyaAsal;
            // Senarai pertukaran milikan tersusun dengan pemunya terbaru di atas sekali
            $senaraiPindah = $ternakan ? $ternakan->senarai_pindah_milik_terkini : collect([$pindahMilik]);
            if ($senaraiPindah->isEmpty()) {
                $senaraiPindah = collect([$pindahMilik]);
            }
