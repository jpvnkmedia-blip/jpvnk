...
@section('title', 'Rekod Aktiviti Baharu - Action List JPVNK')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 p-6 rounded-3xl text-white shadow-xl">
        <div class="space-y-1">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-500/30">
                <i class="fa-solid fa-pen-to-square"></i> BORANG DAIRI & LOG AKTIVITI
            </div>
            <h1 class="text-2xl font-black">Rekod Aktiviti Pegawai / Admin</h1>
            <p class="text-xs text-slate-300">
                Catatkan butiran aktiviti pentadbiran, program lapangan, mesyuarat, atau pemeriksaan premis.
            </p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('action-list.index') }}" class="px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs transition flex items-center gap-1.5 border border-white/20">
                <i class="fa-solid fa-arrow-left"></i> Senarai Aktiviti
            </a>
        </div>
    </div>
    ...
    <form action="{{ route('action-list.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-5">
            ...
            <div class="space-y-4 text-xs">
                <div>
                    ...
                    <input type="text" name="tajuk_aktiviti" value="{{ old('tajuk_aktiviti') }}" required placeholder="cth: Lawatan Pemantauan Projek Ruminan Kg. Gong Chapa / Mesyuarat Penyelarasan Bulanan..." class="w-full py-2.5 px-3.5 rounded-xl border border-slate-300 focus:outline-emerald-500 font-bold text-slate-900 text-sm">
                </div>
                    ...
                    <div>
                        ...
                        <select name="kategori_aktiviti" required class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 font-bold text-slate-800">
                            @foreach($kategoriList as $kKey => $kVal)
                                <option value="{{ $kKey }}" {{ old('kategori_aktiviti') === $kKey ? 'selected' : '' }}>{{ $kVal }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 font-bold text-slate-800">
                            @foreach($statusList as $sKey => $sVal)
                                <option value="{{ $sKey }}" {{ old('status', 'Selesai') === $sKey ? 'selected' : '' }}>{{ $sVal }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="masa_mula" value="{{ old('masa_mula', '09:00 AM') }}" placeholder="cth: 09:00 AM" class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 text-slate-700">
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 uppercase mb-1">Masa Selesai</label>
                        <input type="text" name="masa_selesai" value="{{ old('masa_selesai', '12:30 PM') }}" placeholder="cth: 12:30 PM" class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 text-slate-700">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="lokasi" value="{{ old('lokasi') }}" placeholder="cth: Ladang Ternakan Lembu Jaya, Kg. Gong Chapa..." class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 text-slate-700">
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 uppercase mb-1">Nama Pegawai & Staf Terlibat</label>
                        <input type="text" name="nama_pegawai" value="{{ old('nama_pegawai', Auth::user()->name) }}" placeholder="cth: Dr. Nik Farhan, En. Yusof..." class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 font-semibold text-slate-800">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
            ...
            <div class="space-y-4 text-xs">
                <div>
                    ...
                    <textarea name="maklumat_aktiviti" rows="5" required placeholder="Catatkan perincian aktiviti yang dilaksanakan, perbincangan, penemuan di lapangan, statistik/bilangan ternakan, atau keputusan yang dicapai..." class="w-full py-2.5 px-3.5 rounded-xl border border-slate-300 focus:outline-emerald-500 text-slate-800 leading-relaxed">{{ old('maklumat_aktiviti') }}</textarea>
                </div>
                    ...
                    <div>
                        ...
                        <select name="keutamaan" class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 text-slate-800 font-semibold">
                            @foreach($keutamaanList as $kKeutamaan)
                                <option value="{{ $kKeutamaan }}" {{ old('keutamaan', 'Biasa') === $kKeutamaan ? 'selected' : '' }}>{{ $kKeutamaan }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 uppercase mb-1">Muat Naik Dokumen / Gambar Lampiran (Pilihan)</label>
                        ...
                        <span class="text-[11px] text-slate-400 mt-1 block">Format: JPG, PNG, PDF, Word, Excel. Maksimum 10MB.</span>
                    </div>
                </div>

                <div>
                    <label class="block font-bold text-slate-700 uppercase mb-1">Tindakan Susulan (Jika ada)</label>
                    <textarea name="tindakan_susulan" rows="2" placeholder="Nyatakan sebarang tindakan susulan yang perlu diselesaikan..." class="w-full py-2 px-3 rounded-xl border border-slate-300 focus:outline-emerald-500 text-slate-700">{{ old('tindakan_susulan') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="px-7 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm shadow-lg shadow-emerald-950/20 transition flex items-center gap-2">
                <i class="fa-solid fa-check"></i> Simpan Rekod Aktiviti
            </button>
        </div>
    </form>
</div>
