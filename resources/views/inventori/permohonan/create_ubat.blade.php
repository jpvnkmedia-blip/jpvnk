...
@section('title', 'Borang Permohonan Ubat & Vaksin Veterinar')
@section('page_title', 'Permohonan Ubat, Vaksin & Bekalan Veterinar')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <span class="text-xs font-bold text-rose-700 bg-rose-50 px-3 py-1.5 rounded-xl border border-rose-200">
            <i class="fa-solid fa-pills mr-1"></i> Borang Permohonan Stor Ubat &amp; Farmasi
        </span>
    </div>

    <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-xs">
        <div class="pb-5 mb-6 border-b border-slate-100">
            <h2 class="text-lg font-bold text-slate-900">Borang Pesanan / Permohonan Ubat &amp; Vaksin Veterinar</h2>
            <p class="text-xs text-slate-500 mt-0.5">Sila isi permohonan bekalan farmaseutikal haiwan, vaksin penyakit ternakan, jarum suntikan atau tag telinga EPTR bagi operasi klinik / lapangan.</p>
        </div>

        <form action="{{ route('inventori.permohonan.ubat.store') }}" method="POST" class="space-y-6 text-xs">
            @csrf

            <!-- Maklumat Pemohon -->
            <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">NAMA PEGAWAI / RAWATAN</span>
                ...
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">JAJAHAN / KLINIK BERTUGAS</span>
                    <span class="font-bold text-slate-800 text-xs mt-0.5 block">{{ Auth::user()->jajahan ?? 'Ibu Pejabat Kota Bharu' }}</span>
                </div>
            </div>

            <!-- Pilihan Ubat & Kuantiti -->
            <div class="space-y-4">
                <h3 class="text-sm font-bold text-slate-900 pb-2 border-b border-slate-100 flex items-center gap-2">
                    ...
                    <span>Butiran Ubat / Vaksin Yang Diperlukan</span>
                </h3>

                <div>
                    <label class="block text-[11px] font-bold text-slate-700 uppercase mb-1">Pilih Formula Ubat / Vaksin / Bekalan Rawatan <span class="text-rose-500">*</span></label>
                    <select name="inventori_item_id" required class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-rose-500 focus:outline-none font-semibold">
                        <option value="">-- Sila Pilih Ubat/Vaksin Daripada Stor Farmasi --</option>
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" {{ old('inventori_item_id') == $item->id ? 'selected' : '' }}>
                                [{{ $item->kod_item }}] {{ $item->nama_item }} &bull; (Baki: {{ $item->kuantiti_semasa }} {{ $item->unit }}) &bull; {{ $item->kategori }} &bull; Luput: {{ $item->tarikh_luput ? $item->tarikh_luput->format('d/m/Y') : '-' }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-700 uppercase mb-1">Klinik / Unit / Jajahan Staf <span class="text-rose-500">*</span></label>
                        <input type="text" name="unit_bahagian" value="{{ old('unit_bahagian', 'Klinik Haiwan Jajahan ' . (Auth::user()->jajahan ?? 'Kota Bharu')) }}" required placeholder="cth: Klinik Haiwan Bachok / Unit Rawatan Bergerak" class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 font-medium focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                ...
                <div>
                    <label class="block text-[11px] font-bold text-slate-700 uppercase mb-1">Tujuan Permohonan &amp; Kes Rawatan Ternakan <span class="text-rose-500">*</span></label>
                    <textarea name="tujuan_permohonan" rows="3" required placeholder="Nyatakan tujuan permohonan (cth: Kempen vaksinasi FMD berjadual di jajahan / Rawatan kecemasan penyakit parasit ternakan lembu penternak / Operasi penandaan tag EPTR)..." class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 focus:ring-2 focus:ring-rose-500 focus:outline-none">{{ old('tujuan_permohonan') }}</textarea>
                </div>

                <div>
                    <label class="block text-[11px] font-bold text-slate-700 uppercase mb-1">Catatan Tambahan (Pilihan)</label>
                    <input type="text" name="catatan_pemohon" value="{{ old('catatan_pemohon') }}" placeholder="Sebarang catatan pengendalian rantaian sejuk (cold-chain) atau dos rawatan..." class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 focus:ring-2 focus:ring-rose-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-slate-100 flex items-center justify-between">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold shadow-lg shadow-rose-700/30 transition flex items-center gap-2">
                    ...
                    <span>Hantar Permohonan Ubat / Vaksin</span>
                </button>
            </div>
