...
@section('title', 'Pengurusan Permohonan Ubat & Vaksin Stor Farmasi')
@section('page_title', 'Pengurusan Permohonan Ubat, Vaksin & Bekalan Rawatan')
...
<div class="space-y-6" x-data="{ modalAction: false, selectedPermohonan: null, tindakan: 'lulus', kuantitiDiluluskan: 1, maxStok: 1, unit: '', namaItem: '', namaPemohon: '', noPermohonan: '', permohonanId: null }">
        ...
        <div>
            <div class="flex items-center gap-2">
                <span class="px-2.5 py-1 rounded-lg bg-rose-100 text-rose-900 font-black text-xs uppercase tracking-wider">
                    <i class="fa-solid fa-pills mr-1"></i> Admin Stor Ubat &amp; Farmasi
                </span>
                <span class="text-xs text-slate-500 font-semibold">Semakan Permohonan Klinik Jajahan &amp; Pegawai Rawatan</span>
            </div>
            <h2 class="text-xl font-extrabold text-slate-900 mt-1">Pengurusan Permohonan Ubat &amp; Vaksin</h2>
            <p class="text-xs text-slate-500 mt-0.5">Semak dan luluskan permohonan ubat veterinar, dos vaksin penyakit ternakan, jarum suntikan dan tag EPTR untuk agihan ke jajahan.</p>
        </div>

        <a href="{{ route('inventori.ubat.index') }}" class="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold shadow-md transition flex items-center gap-2">
            <i class="fa-solid fa-arrow-left"></i>
            <span>Kembali ke Stor Ubat</span>
        </a>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            <div class="text-[11px] font-bold text-slate-500 uppercase">Jumlah Permohonan Ubat</div>
            <div class="text-2xl font-black text-slate-900 mt-1">{{ $totalPermohonan }}</div>
            <div class="text-[10px] text-slate-400 mt-0.5">Semua rekod klinik</div>
        </div>
        <div class="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
        ...
        <div class="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            ...
            <div class="text-[10px] text-slate-400 mt-0.5">Ubat telah diagihkan</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 p-4 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex flex-wrap gap-2 text-xs">
            <a href="{{ route('inventori.ubat.permohonan') }}" class="px-3.5 py-1.5 rounded-full font-bold transition {{ !request('status') ? 'bg-slate-900 text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                Semua
            </a>
            <a href="{{ route('inventori.ubat.permohonan', ['status' => 'Menunggu Kelulusan']) }}" class="px-3.5 py-1.5 rounded-full font-bold transition {{ request('status') === 'Menunggu Kelulusan' ? 'bg-amber-600 text-white shadow-xs' : 'bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200' }}">
                ⚠️ Menunggu ({{ $menungguCount }})
            </a>
            <a href="{{ route('inventori.ubat.permohonan', ['status' => 'Diluluskan']) }}" class="px-3.5 py-1.5 rounded-full font-bold transition {{ request('status') === 'Diluluskan' ? 'bg-blue-600 text-white shadow-xs' : 'bg-blue-50 text-blue-800 hover:bg-blue-100 border border-blue-200' }}">
                Diluluskan ({{ $lulusCount }})
            </a>
            <a href="{{ route('inventori.ubat.permohonan', ['status' => 'Telah Diambil / Diserahkan']) }}" class="px-3.5 py-1.5 rounded-full font-bold transition {{ request('status') === 'Telah Diambil / Diserahkan' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                Selesai ({{ $selesaiCount }})
            </a>
            <a href="{{ route('inventori.ubat.permohonan', ['status' => 'Ditolak']) }}" class="px-3.5 py-1.5 rounded-full font-bold transition {{ request('status') === 'Ditolak' ? 'bg-rose-600 text-white shadow-xs' : 'bg-rose-50 text-rose-800 hover:bg-rose-100 border border-rose-200' }}">
                Ditolak
            </a>
        </div>

        <form action="{{ route('inventori.ubat.permohonan') }}" method="GET" class="flex items-center gap-2">
            @if(request('status'))
                <input type="hidden" name="status" value="{{ request('status') }}">
            @endif
            <div class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari pemohon / no. permohonan / ubat..." class="rounded-xl border border-slate-200 pl-8 pr-3 py-1.5 text-xs bg-slate-50 focus:bg-white focus:ring-2 focus:ring-rose-500 focus:outline-none w-56 sm:w-72">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-xs">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            <h3 class="text-sm font-bold text-slate-900">Senarai Permohonan Ubat, Vaksin &amp; Tag EPTR</h3>
                    ...
                    @forelse($permohonans as $p)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="px-5 py-3.5 font-mono">
                                <span class="font-bold text-rose-900 block">{{ $p->no_permohonan }}</span>
                                <span class="text-[10px] text-slate-400">{{ $p->created_at ? $p->created_at->format('d/m/Y H:i') : '-' }}</span>
                            </td>
                            <td class="px-4 py-3.5">
                                <div class="font-bold text-slate-900">{{ $p->pemohon->name ?? 'Pegawai Veterinar' }}</div>
                                <div class="text-[11px] text-slate-500">{{ $p->unit_bahagian }}</div>
                            </td>
                            <td class="px-4 py-3.5">
                                <div class="font-bold text-slate-900">{{ $p->item->nama_item ?? '-' }}</div>
                                <div class="text-[11px] text-slate-500 font-mono">
                                    {{ $p->item->kod_item ?? '-' }} &bull; Batch: {{ $p->item->no_batch ?? 'N/A' }}
                                </div>
                                @if($p->item && $p->item->tarikh_luput)
                                    <div class="text-[10px] {{ $p->item->isExpired() ? 'text-rose-600 font-bold' : 'text-slate-400' }}">
                                        Luput: {{ $p->item->tarikh_luput->format('d/m/Y') }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-4 py-3.5">
                            ...
                            <td class="px-5 py-3.5 text-right whitespace-nowrap space-x-1">
                                ...
                                @elseif($p->isApproved())
                                    ...
                                    <button @click="modalAction = true; permohonanId = {{ $p->id }}; noPermohonan = '{{ $p->no_permohonan }}'; namaPemohon = '{{ addslashes($p->pemohon->name ?? '') }}'; namaItem = '{{ addslashes($p->item->nama_item ?? '') }}'; unit = '{{ $p->item->unit ?? 'Unit' }}'; kuantitiDiluluskan = {{ $p->kuantiti_diluluskan ?? $p->kuantiti_dimohon }}; maxStok = {{ $p->item->kuantiti_semasa ?? 1 }}; tindakan = 'serah'" class="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-xs transition inline-flex items-center gap-1">
                                        <i class="fa-solid fa-box-check"></i> Serah Bekalan
                                    </button>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                                ...
                                <div class="font-bold text-slate-600">Tiada permohonan ubat/vaksin dijumpai</div>
                                <div class="text-xs text-slate-400 mt-0.5">Permohonan baharu daripada klinik jajahan akan dipaparkan di sini.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <!-- MODAL TINDAKAN: Kelulusan / Tolak / Serah Ubat -->
    <div x-show="modalAction" x-cloak class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl text-xs space-y-4" @click.outside="modalAction = false">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center font-bold" :class="tindakan === 'lulus' ? 'bg-blue-100 text-blue-700' : (tindakan === 'serah' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700')">
                        <i class="fa-solid" :class="tindakan === 'lulus' ? 'fa-check' : (tindakan === 'serah' ? 'fa-pills' : 'fa-xmark')"></i>
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-slate-900" x-text="tindakan === 'lulus' ? 'Kelulusan Permohonan Ubat / Vaksin' : (tindakan === 'serah' ? 'Penyerahan & Agihan Bekalan Ubat' : 'Penolakan Permohonan')"></h3>
            ...
            <div class="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
                <div>Pegawai/Klinik: <b class="text-slate-900" x-text="namaPemohon"></b></div>
                <div>Formula Ubat/Vaksin: <b class="text-rose-900" x-text="namaItem"></b></div>
                <div class="text-[11px] text-slate-500">Baki Stok Farmasi: <b class="text-slate-800" x-text="maxStok + ' ' + unit"></b></div>
            </div>
            ...
            <form :action="'{{ url('/inventori/permohonan') }}/' + permohonanId + '/status'" method="POST" class="space-y-4">
                ...
                <template x-if="tindakan === 'lulus' || tindakan === 'serah'">
                    <div>
                        <label class="block font-bold text-slate-700 uppercase mb-1">Kuantiti Diluluskan / Diagihkan (<span x-text="unit"></span>) <span class="text-rose-500">*</span></label>
                        <input type="number" name="kuantiti_diluluskan" x-model="kuantitiDiluluskan" min="1" :max="maxStok" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                </template>

                <div>
                    <label class="block font-bold text-slate-700 uppercase mb-1" x-text="tindakan === 'tolak' ? 'Sebab Penolakan Permohonan' : 'Arahan Pegawai Stor Farmasi (Pilihan)'"></label>
                    <textarea name="catatan_pegawai" rows="2.5" :required="tindakan === 'tolak'" :placeholder="tindakan === 'tolak' ? 'Nyatakan sebab penolakan (cth: Stok terhad untuk kes kecemasan jajahan lain)...' : 'Catatan dos atau arahan penyimpanan rantaian sejuk...' " class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500 focus:outline-none"></textarea>
                </div>

                <template x-if="tindakan === 'serah'">
                    <div class="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-emerald-800 text-[11px]">
                        <i class="fa-solid fa-circle-info mr-1"></i> Penyerahan bekalan ubat ini akan <b>menolak baki stok farmasi secara automatik</b> dan merekodkan transaksi <b>Stok Keluar</b> dalam lejar.
                    </div>
                </template>
                ...
                <div class="pt-3 flex justify-end gap-2">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-xl text-white font-bold shadow-md transition flex items-center gap-1.5" :class="tindakan === 'lulus' ? 'bg-blue-600 hover:bg-blue-700' : (tindakan === 'serah' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700')">
                        ...
                        <span x-text="tindakan === 'lulus' ? 'Sahkan Kelulusan' : (tindakan === 'serah' ? 'Sahkan Penyerahan Ubat' : 'Sahkan Penolakan')"></span>
                    </button>
                </div>
            </form>
        </div>
